...
def level_1():
	...
	delta_data = 0.1
	enemy.name = "哥布林"
	enemy.hp = E_HP = 420 * frandomly(delta_data)
	enemy.atk = E_ATK = 40 * frandomly(delta_data)
...
def level_2():
	...
	delta_data = 0.1
	enemy.name = "哥布林国王"
	enemy.hp = E_HP = 1200 * frandomly(delta_data)
	enemy.atk = E_ATK = 100 * frandomly(delta_data)
...
def start(level):
	...
	while(enemy.hp > 0 and lead.hp > 0):
		...
		# 击败	
			...
			print("\n你击败了" + enemy.name + "。")
			basic = level * 999  # 掉宝质量
			basic_cre = level * 999
			crhhigh = 999  # 暴率上限
			thphigh = 999  # 穿透上限
			fhphigh = 999  # 吸血上限
			mishigh = 999  # 闪避上限
			hundred_crh = level  # 百分比暴击掉宝
			hundred_thp = level  # 百分比穿透掉宝
			hundred_fhp = level  # 百分比吸血掉宝
			hundred_mis = level  # 百分比闪避掉宝
			if level < 10:
