		...
		# 击败	
			...
			if level < 10:
				...
				if probability(level / 25):
					...
					if gem == "暴击率":
						out_gem += hundred_crh_gem
					if gem == "暴击效果":
						out_gem = CRE * basic_gem
					if gem == "回血":
						out_gem = RHP * basic_gem
					if gem == "穿透":
						out_gem = hundred_thp_gem
					if gem == "吸血":
						out_gem = hundred_fhp_gem
					if gem == "闪避":
						out_gem = hundred_mis_gem
					basic_fgem = basic * round(uniform(-3, -6), 2)
					hundred_crh_fgem = hundred_crh * round(uniform(-3, -6), 2)
					hundred_thp_fgem = hundred_thp * round(uniform(-3, -6), 2)
					hundred_fhp_fgem = hundred_fhp * round(uniform(-3, -6), 2)
					hundred_mis_fgem = hundred_mis * round(uniform(-3, -6), 2)
					if fgem == "血量":
					...
					if fgem == "暴击率":
						out_fgem = hundred_crh_fgem
					if fgem == "暴击效果":
						out_fgem = CRE * basic_fgem
					if fgem == "回血":
						out_fgem = RHP * basic_fgem
					if fgem == "穿透":
						out_fgem = hundred_thp_fgem
					if fgem == "吸血":
						out_fgem = hundred_fhp_fgem
					if fgem == "闪避":
						out_fgem = hundred_mis_fgem

					print("检测成功! ")
					sleep(interval)
					print("其正属性为:", gem + "，增加", abs(out_gem), "点")
					sleep(interval)
					print("其负属性为:", fgem + "，减少", abs(out_fgem), "点")
					sleep(interval)
...
print("——————————————————————————————————————")
sleep(interval)
print("1.修复了斩杀技能效果异常的 bug。")
sleep(interval)
print("2.优化了显示体验。")
sleep(interval)
print("3.修复了宝石属性异常的 bug。")
sleep(interval)
print("4.修复了彩蛋触发异常的 bug。")
sleep(interval)
