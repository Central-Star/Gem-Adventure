	...
	if con:
		if lead.hp > 0:
			...
			if egg == "Tell Me More":
				...
				print("你说。")
				sleep(interval)
				print("那个 “推选”……是什么时候进行的? ")
				sleep(interval)
				print("“推选”? 呵，我从来就没进行过。")
				sleep(interval)
				print("那……")
				sleep(interval)
				print("众人看到生活的希望，便会将其顶上最高峰使其难以下台，至于 “推选”……他们不会在意，正如 “凛冬” 一样冰冷，而那位勇士，则是我早已定下的。")
				sleep(interval)
				print("那位勇士……是您定下的?! ")
				sleep(interval)
				print("无论结局与否，这都是他必须经历的，而所谓的不灭之源……是他的前世，亦是他的未来，所以谓之 “不灭”。")
				sleep(interval)
				print("……")
				sleep(interval)
				print("只有他自己，才能打败他自己……")
