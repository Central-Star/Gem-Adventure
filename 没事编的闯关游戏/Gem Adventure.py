...
# 创建浮动机制
def frandomly(delta):
	return round(uniform(1 - delta, 1 + delta), 2)
...
def prppt(obj):
	...
	if obj == 0:
		...
		if E_MIS == 0.999:
			...
			print("血量:", round(E_HP, 2))
			print("攻击:", round(E_ATK, 2))
			print("防御:", round(E_DFC, 2))
			print("暴击率:", round(E_CRH * 100, 2), "%")
			print("暴击效果:", round(E_CRE, 2))
			print("回血:", round(E_RHP, 2))
...
def printskill():
	...
	print("——————————————————————————————————————")
	print("消耗 1 能量: ")
	print("0.【恢复】 回复三倍回血的血量。")
	print("1.【强化】 攻击增加 50%，持续 1 回合。")
	print("2.【守护】 防御增加 80%，持续 1 回合。")
	print("3.【斩杀】 对目标造成 20% 目标已损生命值的伤害。")
	print("消耗 2 能量: ")
	print("4.【圣光】 增加 300% 回血，减少目标 50% 的防御，持续 2 回合。")
	print("5.【诡步】 增加 50% 穿透，15% 暴击率，30% 闪避，持续 2 回合。")
	print("消耗 3 能量: ")
	print("6.【狂暴】 增加 40% 穿透，5% 吸血，30% 暴击率，75% 攻击，持续 3 回合。")
	print("7.【压制】 使目标无法防御，无法闪避，持续 3 回合。")
	print("——————————————————————————————————————\n")
	sleep(interval)
...
def level_1():
	...
	delta_data = 0.1
	enemy.name = "哥布林"
	enemy.hp = E_HP = 320 * frandomly(delta_data)
	enemy.atk = E_ATK = 30 * frandomly(delta_data)
	enemy.dfc = E_DFC = 12 * frandomly(delta_data)
	enemy.crh = E_CRH = 0
	enemy.cre = E_CRE = 0
	enemy.rhp = E_RHP = 0
	enemy.thp = E_THP = 0
...
def level_2():
	...
	delta_data = 0.1
	enemy.name = "哥布林国王"
	enemy.hp = E_HP = 900 * frandomly(delta_data)
	enemy.atk = E_ATK = 80 * frandomly(delta_data)
	enemy.dfc = E_DFC = 40 * frandomly(delta_data)
	enemy.crh = E_CRH = 0.07 * frandomly(delta_data)
	enemy.cre = E_CRE = 2 * frandomly(delta_data)
	enemy.rhp = E_RHP = 14 * frandomly(delta_data)
	enemy.thp = E_THP = 0.04 * frandomly(delta_data)
	enemy.fhp = E_FHP = 0
	enemy.mis = E_MIS = 0
...
def level_3():
	...
	delta_data = 0.1
	enemy.name = "羊头人"
	enemy.hp = E_HP = 2800 * frandomly(delta_data)
	enemy.atk = E_ATK = 200 * frandomly(delta_data)
	enemy.dfc = E_DFC = 80 * frandomly(delta_data)
	enemy.crh = E_CRH = 0.2 * frandomly(delta_data)
	enemy.cre = E_CRE = 2.8 * frandomly(delta_data)
	enemy.rhp = E_RHP = 48 * frandomly(delta_data)
	enemy.thp = E_THP = 0.09 * frandomly(delta_data)
	enemy.fhp = E_FHP = 0
	enemy.mis = E_MIS = 0.04 * frandomly(delta_data)
...
def level_4():
	...
	delta_data = 0.1
	enemy.name = "卓柏卡布拉"
	enemy.hp = E_HP = 6000 * frandomly(delta_data)
	enemy.atk = E_ATK = 350 * frandomly(delta_data)
	enemy.dfc = E_DFC = 100 * frandomly(delta_data)
	enemy.crh = E_CRH = 0.3 * frandomly(delta_data)
	enemy.cre = E_CRE = 4 * frandomly(delta_data)
	enemy.rhp = E_RHP = 180 * frandomly(delta_data)
	enemy.thp = E_THP = 0.24 * frandomly(delta_data)
	enemy.fhp = E_FHP = 0.75 * frandomly(delta_data)
	enemy.mis = E_MIS = 0.08 * frandomly(delta_data)
...
def level_5():
	...
	delta_data = 0.1
	enemy.name = "深渊之主"
	enemy.hp = E_HP = 10200 * frandomly(delta_data)
	enemy.atk = E_ATK = 520 * frandomly(delta_data)
	enemy.dfc = E_DFC = 275 * frandomly(delta_data)
	enemy.crh = E_CRH = 0.5 * frandomly(delta_data)
	enemy.cre = E_CRE = 5 * frandomly(delta_data)
	enemy.rhp = E_RHP = 200 * frandomly(delta_data)
	enemy.thp = E_THP = 0.4 * frandomly(delta_data)
	enemy.fhp = E_FHP = 0.5 * frandomly(delta_data)
	enemy.mis = E_MIS = 0.3 * frandomly(delta_data)
...
def level_6():
	...
	delta_data = 0.1
	enemy.name = "变形幽灵"
	enemy.hp = E_HP = 23720 * frandomly(delta_data)
	enemy.atk = E_ATK = 1310 * frandomly(delta_data)
	enemy.dfc = E_DFC = 695 * frandomly(delta_data)
	enemy.crh = E_CRH = 0.6 * frandomly(delta_data)
	enemy.cre = E_CRE = 6.4 * frandomly(delta_data)
	enemy.rhp = E_RHP = 284 * frandomly(delta_data)
	enemy.thp = E_THP = 0.45 * frandomly(delta_data)
	enemy.fhp = E_FHP = 0.5 * frandomly(delta_data)
	enemy.mis = E_MIS = 0.85 * frandomly(delta_data)
...
def level_7():
	...
	delta_data = 0.1
	enemy.name = "血魔猎手"
	enemy.hp = E_HP = 45500 * frandomly(delta_data)
	enemy.atk = E_ATK = 3360 * frandomly(delta_data)
	enemy.dfc = E_DFC = 1550 * frandomly(delta_data)
	enemy.crh = E_CRH = 0.8 * frandomly(delta_data)
	enemy.cre = E_CRE = 8.5 * frandomly(delta_data)
	enemy.rhp = E_RHP = 530 * frandomly(delta_data)
	enemy.thp = E_THP = 0.85 * frandomly(delta_data)
	enemy.fhp = E_FHP = 0.55 * frandomly(delta_data)
	enemy.mis = E_MIS = 0.6 * frandomly(delta_data)
...
def level_8():
	...
	delta_data = 0.1
	enemy.name = "恶魔之眼"
	enemy.hp = E_HP = 87500 * frandomly(delta_data)
	enemy.atk = E_ATK = 5550 * frandomly(delta_data)
	enemy.dfc = E_DFC = 2350 * frandomly(delta_data)
	enemy.crh = E_CRH = 0.9 * frandomly(delta_data)
	enemy.cre = E_CRE = 15 * frandomly(delta_data)
	enemy.rhp = E_RHP = 12550 * frandomly(delta_data)
	enemy.thp = E_THP = 0.9 * frandomly(delta_data)
	enemy.fhp = E_FHP = 0.8 * frandomly(delta_data)
	enemy.mis = E_MIS = 0.5 * frandomly(delta_data)
...
def level_9():
	...
	delta_data = 0.1
	enemy.name = "暗黑玄武"
	enemy.hp = E_HP = 160000 * frandomly(delta_data)
	enemy.atk = E_ATK = 12600 * frandomly(delta_data)
	enemy.dfc = E_DFC = 16000 * frandomly(delta_data)
	enemy.crh = E_CRH = 0.9 * frandomly(delta_data)
	enemy.cre = E_CRE = 20 * frandomly(delta_data)
	enemy.rhp = E_RHP = 10000 * frandomly(delta_data)
	enemy.thp = E_THP = 1
	enemy.fhp = E_FHP = 0.2 * frandomly(delta_data)
	enemy.mis = E_MIS = 0
...
def start(level):
	...
	global both  # 一次性彩蛋判断
	con = False  # 未通关
	attribute_list = ["血量", "攻击", "防御", "暴击率", "暴击效果", "回血", "穿透", "吸血", "闪避"]
	skill_list = ["恢复", "强化", "守护", "斩杀", "圣光", "诡步", "狂暴", "压制"]
	if level < 10:
		setup()
	else:
		if lead_name != "不灭":
	...
	while enemy.hp > 0 and lead.hp > 0:
		...
		if lead.mp >= 1:
			...
			skill_statue_test = " "
			skip = False
			while (skill_statue > 7 or skill_statue < 0 or skill_statue_test != "") and skip == False:
				skill_statue_test = input("◈请使用技能（按回车键跳过）: ")
				if skill_statue_test != "":
					if not skill_statue_test.isdigit():
						print("你的输入不符合规则，请输入技能编号! ")
					else:
						skill_statue = int(skill_statue_test)
						if skill_statue > 7 or skill_statue < 0:
							print("你所输入的技能编号错误，请输入正确的技能编号! ")
						else:
							skill_statue_test = ""
				else:
					skip = True
		...
		# 计算伤害
		lead_atp = (lead.atk - enemy.dfc * (1 - lead.thp)) * round(uniform(1 - level * 0.015, 1 + level * 0.01), 2)
		if lead_atp < 0:
			lead_atp = 0
		enemy_atp = (enemy.atk - lead.dfc * (1 - enemy.thp)) * round(uniform(1 - level * 0.01, 1 + level * 0.015), 2)
		if enemy_atp < 0:
			enemy_atp = 0
		...
		# 击败	
			...
			print("\n你击败了" + enemy.name + "。")
			basic = level * 0.06  # 掉宝质量
			basic_cre = level * 0.02
			crhhigh = 0.6  # 暴率上限
			thphigh = 0.5  # 穿透上限
			fhphigh = 0.5  # 吸血上限
			mishigh = 0.3  # 闪避上限
			hundred_crh = level * (crhhigh / 35)  # 百分比暴击掉宝
			hundred_thp = level * (thphigh / 35)  # 百分比穿透掉宝
			hundred_fhp = level * (fhphigh / 35)  # 百分比吸血掉宝
			hundred_mis = level * (mishigh / 35)  # 百分比闪避掉宝
			if level < 10:

				# 属性点掉宝
				if probability(level / 5):
					ppt = " "
					while ppt not in attribute_list:
				...
				if probability(level / 25):
					...
					sleep(interval)
					have_statue = ""
					while have_statue != "是" and have_statue != "否":
...
sleep(interval)
print("可国王声称你通过了 “推选”，仍赋予了你 “弑魔勇士” 的重任，在众人希望的目光下，你不得不前去讨伐不灭之源……")
sleep(interval)
...
sleep(interval)
print("4.每一次攻击都不一定正中下怀，每次对怪物的伤害皆有一定的浮动，特别是后面关卡的怪物。")
sleep(interval)
	...
	if con:
		if lead.hp > 0:
			...
			if egg == "Hello World":
				...
				sleep(interval)
				print("后续会增加更多有意思的机制，敬请期待! 作者邮箱: 3584228792@qq.com")
				sleep(interval)
				print("等等，我又有一个小提示 (坏笑)")
				sleep(interval)
				print("以后通关记得试试输个“Tell Me More”，说不定有惊喜哦……")
				sleep(interval)
				print("◈按下回车键结束游戏……")
				input()
			elif egg == "Tell Me More":
				print("恭喜你发现了 【轮回】 彩蛋! ")
				sleep(interval)
				print("千年过去了……")
				sleep(interval)
				...
				sleep(interval)
				print("你不会以为这个游戏就这么结束了吧? ")
				sleep(interval)
				print("既然你玩到了这，给你个小提示! ")
				sleep(interval)
				print("通关时输入“Hello World”，直达作者老家! ")
				sleep(interval)
				print("◈按下回车键结束游戏……")
				input()
			break
			...
			if lead_name == "不灭":
				...
				if egg == "Tell Me More":
					...
					print("你说。")
					sleep(interval)
					print("那个 “推选”……是什么时候进行的? 我为什么不知道? ")
					sleep(interval)
					print("“推选”? 呵，我从来就没进行过，借口罢了。")
					sleep(interval)
					print("那……")
					sleep(interval)
					print("那位勇士……已是上一任国王定下的。")
					sleep(interval)
					print("那位勇士……是祂定下的?! ")
					sleep(interval)
					...
					print("……")
					sleep(interval)
					print("我现在，便是在做祂曾经做过的事，也是在等，等他幡然醒悟——")
					sleep(interval)
					print("……")
					sleep(interval)
